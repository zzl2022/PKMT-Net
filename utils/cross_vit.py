...
class MultiScaleEncoder(nn.Module):
    ...
    def forward(self, sm_tokens, lg_tokens):
        for i, (sm_enc, lg_enc, cross_attend) in enumerate(self.layers):
            sm_tokens_enc, lg_tokens_enc = sm_enc(sm_tokens), lg_enc(lg_tokens)
            sm_tokens, lg_tokens = cross_attend(sm_tokens_enc, lg_tokens_enc)

            # Adding residual connection
            sm_tokens += sm_tokens_enc
            lg_tokens += lg_tokens_enc
...
class ImageEmbedder(nn.Module):
    def __init__(
        ...
        *,
        ...
        # 软分割
        self.to_patch_embedding = nn.Sequential(
            nn.Conv2d(3, dim, kernel_size=patch_size, stride=(patch_size-2)),
            Rearrange('b e (h) (w) -> b (h w) e'),
            nn.LayerNorm(dim),
        )

        # self.pos_embedding = nn.Parameter(
        #     torch.randn(1, int((image_size - patch_size) / (patch_size - 2) + 1) ** 2 + 1, dim))
    ...
    def forward(self, img):
        ...
        x += positional_encoding(seq_len=x.shape[1], d_model=x.shape[2]).to(x.device)  # fixed
...
class CrossViT(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.w = nn.Parameter(torch.Tensor([0.5]))  # Initialize with 0.5
    ...
    def forward(self, img):
        ...
        w = 0.3
        weighted_logits = w * lg_logits + (1 - w) * sm_logits
